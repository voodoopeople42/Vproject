...
from .phonebook02 import Phonebook
import json
...
class App:
    def __init__(self, master):
        ...
        self.contacts = self.phonebook.load_data()
    ...
    def search_command(self):
        searchFor = self.e_name.get()
        for item in self.contacts:
            k = self.search(item, searchFor)
            if k:
                print(item)
    
    def view_command(self):
        self.li.delete(0, END)
        for row in self.phonebook.load_data():
    ...
    def add_command(self):
        ...
        self.contacts.append(json.loads(contact.toJson()))
        self.phonebook.save(self.contacts)
    ...
    def get_selected_row(self,event):
        try:
            self.index = self.li.curselection()[0]
            self.selected_tuple = self.li.get(self.index)
